...
class Detector:
    ...
    def procesar(self):
        ...
        while True:
            ...
            self.procesar_deteccion(cuadro)
    ...
    def procesar_deteccion(self, cuadro):
        pass


class DetectorYOLO(Detector):
    def __init__(self, fuente, metodo, tiempo_maximo, modelo_haar=None):
        super().__init__(fuente, metodo, tiempo_maximo, modelo_haar)
    ...
    def procesar_deteccion(self, cuadro):
        ...
        blob = cv2.dnn.blobFromImage(cuadro, 1 / 255.0, (320, 320), swapRB=True, crop=False)
        ...
        for salida in salidas:
            for deteccion in salida:
                ...
                if confianza > 0.3 and id_clase in self.clases_interes:  
        ...
        if len(idx) > 0:
            for i in idx.flatten():
                x, y, w, h = cajas[i]
                color = [int(c) for c in self.colores[ids_clase[i]]]
                cv2.rectangle(cuadro, (x, y), (x + w, y + h), color, 2)
                texto = f"{self.etiquetas[ids_clase[i]]}: {confidencias[i]:.2f}"
                cv2.putText(cuadro, texto, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
...
class DetectorHaar(Detector):
    def __init__(self, fuente, metodo, tiempo_maximo, modelo_haar):
        super().__init__(fuente, metodo, tiempo_maximo, modelo_haar)
        self.clasificador_rostros = cv2.CascadeClassifier(f'model/Haar/{modelo_haar}')
    ...
    def procesar_deteccion(self, cuadro):
        ...
        if len(rostros) > 0:
            ...
            for (x, y, w, h) in rostros:
                ...
                cv2.putText(cuadro, " ", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
...
class App:
    def __init__(self, master):
        ...
        self.fondo_imagen = Image.open("fondo.jpg")
        self.fondo_imagen = ImageTk.PhotoImage(self.fondo_imagen)
        self.fondo_label = tk.Label(master, image=self.fondo_imagen)
        ...
        self.ip_label = tk.Label(self.frame, text="Cámara IP (si aplica):")
        self.ip_label.pack()
        ...
        self.camara_local_label = tk.Label(self.frame, text="Índice Cámara Local:")
        self.camara_local_label.pack()

        self.camara_local_entry = tk.Entry(self.frame)
        self.camara_local_entry.pack()
        self.camara_local_entry.insert(0, "0")  
        self.tiempo_label = tk.Label(self.frame, text="Tiempo máximo sin detección (segundos):")
        self.tiempo_label.pack()

        self.tiempo_entry = tk.Entry(self.frame)
        self.tiempo_entry.pack()
        self.tiempo_entry.insert(0, "10")  

        self.iniciar_button = tk.Button(self.frame, text="Iniciar Detección", command=self.iniciar_deteccion)
        self.iniciar_button.pack()

        self.iniciar_camara_local_button = tk.Button(self.frame, text="Iniciar Cámara Local", command=self.iniciar_camara_local)
        self.iniciar_camara_local_button.pack(pady=10)
        ...
        self.borrar_icon = Image.open("trash_icon.png")
        self.borrar_icon = self.borrar_icon.resize((20, 20), Image.Resampling.LANCZOS)
        self.borrar_icon = ImageTk.PhotoImage(self.borrar_icon)

        self.borrar_credenciales_button.config(image=self.borrar_icon, compound=tk.LEFT)

        self.iniciar_button.bind("<Enter>", self.highlight_iniciar_button)
        self.iniciar_button.bind("<Leave>", self.remove_highlight_iniciar_button)
        self.borrar_credenciales_button.bind("<Enter>", self.highlight_borrar_button)
        self.borrar_credenciales_button.bind("<Leave>", self.remove_highlight_borrar_button)

    def highlight_iniciar_button(self, event):
        self.iniciar_button.config(bg="blue", fg="white")

    def remove_highlight_iniciar_button(self, event):
        self.iniciar_button.config(bg="SystemButtonFace", fg="black")

    def highlight_borrar_button(self, event):
        self.borrar_credenciales_button.config(bg="red", fg="white")

    def remove_highlight_borrar_button(self, event):
        self.borrar_credenciales_button.config(bg="SystemButtonFace", fg="black")
    ...
    def iniciar_deteccion(self):
        ...
        fuente_video = ip_camara if ip_camara else 0  
        if metodo_deteccion == "YOLO":
            detector = DetectorYOLO(fuente_video, metodo_deteccion, tiempo_maximo_sin_deteccion, modelo_haar)
        else:
            detector = DetectorHaar(fuente_video, metodo_deteccion, tiempo_maximo_sin_deteccion, modelo_haar)

        self.detector_thread = Thread(target=detector.procesar, daemon=True)
        self.detector_thread.start()

    def iniciar_camara_local(self):
        try:
            indice_camara = int(self.camara_local_entry.get())
            fuente_video = indice_camara  
            ...
            messagebox.showerror("Error", "Por favor ingrese un índice válido para la cámara local.")
